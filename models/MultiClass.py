...
class MultiTaskSeq2SeqTrainer(Seq2SeqTrainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.args.weight_mode == 'bias' or self.args.weight_mode == 's2t':
            loss = loss_t2s + 0.1 * loss_s2t
        else:
            loss = loss_t2s + loss_s2t

        return (loss, (loss_t2s, loss_s2t), (outputs_t2s, outputs_s2t)) if return_outputs else (loss, (loss_t2s, loss_s2t))
    ...
    def prediction_loop(
        ...
        losses_host: List[torch.Tensor] = [None, None]
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                ...
                losses_host, preds_host_user, preds_host_agent, labels_host_user, labels_host_agent = [None,
        ...
        if eval_loss_topic is not None:
            ...
            metrics[
                f"{metric_key_prefix}_loss_total"] = eval_loss_sum.mean().item() + 0.1 * eval_loss_topic.mean().item()
...
class TopicVocab:
    def __init__(self, data_files, embedding_size):
        self.topic = {}
        for file in data_files:
            with open(file, encoding='utf-8') as f:
                content = json.load(f)
                for sample in content:
                    if sample['Topic'] not in self.topic.keys():
                        self.topic2[sample['Topic']] = len(self.topic)
        self.vocab_size = len(self.topic)
