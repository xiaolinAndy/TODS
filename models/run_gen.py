...
parser.add_argument("--dataset", default="CSDS",type=str)
...
parser.add_argument("--gradient_accumulation_steps",default="6",type=str)
...
def preprocess_function(examples):
    ...
    for i in range(len(inputs)):
        ...
        model_inputs['input_ids'].append(input_ids
                                             + topics['input_ids'][i])
        model_inputs['attention_mask'].append([1] * len(model_inputs['input_ids'][-1]))
...
class TestCallback(TrainerCallback):
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        metrics = trainer.evaluate(test_dataset, metric_key_prefix="eval")
