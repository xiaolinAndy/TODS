...
class DataCollatorForMultiTaskSeq2Seq:
    ...
    sum_start_id: int = 50260,
    topic_start_id: int = 50261,
    ...
    def __call__(self, features):
        ...
        input_ids_right = [feature["input_ids_t2s_right"] for feature in features] if "input_ids_t2s_right" in features[0].keys() else None
        input_ids_wrong = [feature["input_ids_t2s_wrong"] for feature in features] if "input_ids_t2s_wrong" in features[0].keys() else None
        ...
        if labels_t2s is not None:
            ...
            max_input_length = max(max(len(l) for l in input_ids_right), max(len(l) for l in input_ids_wrong))
            ...
            for feature in features:
                ...
                # pad inputs
                remainder_t2s_right = [self.input_pad_token_id] * (max_input_length - len(feature["input_ids_t2s_right"]))
                feature["input_ids_t2s_right"] = (
                    feature["input_ids_t2s_right"] + remainder_t2s_right if padding_side == "right" else remainder_t2s_right + feature[
                        "input_ids_t2s_right"]
                )
                remainder_t2s_wrong = [self.input_pad_token_id] * (
                            max_input_length - len(feature["input_ids_t2s_wrong"]))
                feature["input_ids_t2s_wrong"] = (
                    feature["input_ids_t2s_wrong"] + remainder_t2s_wrong if padding_side == "right" else remainder_t2s_wrong + feature[
                        "input_ids_t2s_wrong"]
                )
                ...
                # pad attention mask
                feature["attention_mask_t2s_right"] = feature["attention_mask_t2s_right"] + remainder_t2s_right
                feature["attention_mask_t2s_wrong"] = feature["attention_mask_t2s_wrong"] + remainder_t2s_wrong
                feature["attention_mask_s2t"] = feature["attention_mask_s2t"] + remainder_s2t
                feature["att_strict"] = feature["att_strict"] + remainder_t2s_right
        ...
        if self.model is not None and hasattr(self.model, "prepare_decoder_input_ids_from_labels"):
            decoder_input_ids_t2s = shift_tokens_right(input_ids=features['labels_t2s'], pad_token_id=self.input_pad_token_id,
                                                       decoder_start_token_id=self.sum_start_id)
            decoder_input_ids_s2t = shift_tokens_right(input_ids=features['labels_s2t'], pad_token_id=self.input_pad_token_id,
                                                       decoder_start_token_id=self.topic_start_id)
            features["decoder_input_ids_t2s"] = decoder_input_ids_t2s
            features["decoder_input_ids_s2t"] = decoder_input_ids_s2t
...
class MultiTaskSeq2SeqTrainer(Seq2SeqTrainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # exit()
        outputs_right = model(input_ids=inputs['input_ids_t2s_right'],
                            decoder_input_ids=inputs['decoder_input_ids_t2s'],
                            attention_mask=inputs['attention_mask_t2s_right'],
                            labels=inputs['labels_t2s'],
                            output_attentions=True)
        outputs_wrong = model(input_ids=inputs['input_ids_t2s_wrong'],
                            decoder_input_ids=inputs['decoder_input_ids_t2s'],
                            attention_mask=inputs['attention_mask_t2s_wrong'],
                            labels=inputs['labels_t2s'])
        # outputs_wrong_s2t = model(input_ids=inputs['input_ids_t2s_right'],
        ...
        outputs_s2t = model(input_ids=inputs['input_ids_s2t'],
                            ...
                            labels=inputs['labels_s2t'])
        cross_att = torch.mean(torch.mean(outputs_right.cross_attentions[-1], dim=2),
        ...
        lm_logits_right = outputs_right['logits']  # (bs, seq_len, vocab_size)
        masked_lm_logits_right = loss_fct(lm_logits_right.view(-1, self.model.config.vocab_size),
                                          inputs['labels_t2s'].view(-1))
        loss_mask_right = torch.ne(inputs['labels_t2s'], -100)
        masked_lm_logits_right = torch.sum(masked_lm_logits_right.view(inputs['labels_t2s'].shape[0], -1),
                                           dim=-1) / torch.sum(loss_mask_right, dim=-1)  # (bs)]

        lm_logits_wrong = outputs_wrong['logits']  # (bs, seq_len, vocab_size)
        masked_lm_logits_wrong = loss_fct(lm_logits_wrong.view(-1, self.model.config.vocab_size),
                                          inputs['labels_t2s'].view(-1))
        loss_mask_wrong = torch.ne(inputs['labels_t2s'], -100)
        masked_lm_logits_wrong = torch.sum(masked_lm_logits_wrong.view(inputs['labels_t2s'].shape[0], -1),
                                           dim=-1) / torch.sum(loss_mask_wrong, dim=-1)  # (bs)]
        loss_shannon = torch.mean(nn.ReLU()(masked_lm_logits_right - masked_lm_logits_wrong + 0.1) * inputs['shannon_mask'])

        # lm_logits_wrong_topic = outputs_wrong_s2t['logits']
        # masked_lm_logits_wrong_topic = loss_fct(lm_logits_wrong_topic.view(-1, self.model.config.vocab_size),
        #                                   inputs['labels_s2t'].view(-1))
        # loss_mask_wrong_topic = torch.ne(inputs['labels_s2t'], -100)
        # masked_lm_logits_wrong_topic = torch.sum(masked_lm_logits_wrong_topic.view(inputs['labels_s2t'].shape[0], -1),
        #                                    dim=-1) / torch.sum(loss_mask_wrong_topic, dim=-1)
        # loss_shannon_topic = torch.mean(
        #     nn.ReLU()(masked_lm_logits_right - masked_lm_logits_wrong_topic + 0.5))


        loss_s2t = outputs_s2t["loss"] if isinstance(outputs_s2t, dict) else outputs_s2t[0]
        loss_att = torch.mean(torch.sum(cross_att, dim=-1))
        loss_right = outputs_right['loss']
        loss = loss_right + 0.2 * loss_s2t + loss_att + loss_shannon# + loss_shannon_topic
        #loss = loss_right + loss_att + loss_shannon


        return (loss, (loss_right, loss_s2t, loss_att, loss_shannon), (outputs_right, outputs_s2t)) if return_outputs else (loss, (loss_right, loss_s2t, loss_att, loss_shannon))
    ...
    def prediction_loop(
        ...
        losses_host: List[torch.Tensor] = [None, None, None, None]
        ...
        eval_losses_gatherer_shannon = DistributedTensorGatherer(world_size, num_examples, make_multiple_of=batch_size)
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                ...
                eval_losses_gatherer_shannon.add_arrays(self._gather_and_numpify(losses_host[3], "eval_shannon_losses"))
                if not prediction_loss_only:
                ...
                llosses_host, preds_host_user, preds_host_agent, labels_host_user, labels_host_agent = [None, None, None, None], None, None, None, None
        ...
        eval_losses_gatherer_topic.add_arrays(self._gather_and_numpify(losses_host[1], "eval_topic_losses"))
        eval_losses_gatherer_att.add_arrays(self._gather_and_numpify(losses_host[2], "eval_att_losses"))
        eval_losses_gatherer_shannon.add_arrays(self._gather_and_numpify(losses_host[3], "eval_att_losses"))
        if not prediction_loss_only:
            preds_gatherer_user.add_arrays(self._gather_and_numpify(preds_host_user, "eval_preds_user"))
            preds_gatherer_agent.add_arrays(self._gather_and_numpify(preds_host_agent, "eval_preds_agent"))
            labels_gatherer_user.add_arrays(self._gather_and_numpify(labels_host_user, "eval_label_ids_user"))
        ...
        eval_loss_shannon = eval_losses_gatherer_shannon.finalize()
        ...
        if eval_loss_topic is not None:
            ...
            # new_loss = []
            # for i in range(len(eval_loss_topic)):
            #     new_loss.append(max(eval_loss_sum[i] - eval_loss_topic[i] + 0.5, 0))
            # new_loss = np.array(new_loss).mean()
            # metrics[
            #     f"{metric_key_prefix}_loss_total"] = new_loss
        if eval_loss_att is not None:
            metrics[f"{metric_key_prefix}_loss_att"] = eval_loss_att.mean().item()
        if eval_loss_shannon is not None:
            metrics[f"{metric_key_prefix}_loss_shannon"] = eval_loss_shannon.mean().item()
            metrics[
                f"{metric_key_prefix}_loss_total"] = eval_loss_sum.mean().item() + 0.2 * eval_loss_topic.mean().item() + eval_loss_att.mean().item() + eval_loss_shannon.mean().item()
